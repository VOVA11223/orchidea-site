            ...
            {saleProducts.map(p => (
              <div key={p.id} className="flex flex-col h-full bg-card rounded-2xl border border-border overflow-hidden hover:-translate-y-1 hover:shadow-lg transition-all duration-200">
                ...
                <div className="flex flex-col gap-1 px-3 pt-3">
                  ...
                  <Link href={`/product/${p.id}`} className="text-sm font-medium text-foreground hover:text-primary-600 transition-colors line-clamp-2 leading-snug min-h-[2.5rem] md:min-h-0">
                    {p.name}
                  </Link>
